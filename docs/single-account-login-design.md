...
当前业务要求改为：同一账号同一时间只能有一个 Keystone 在线 access 会话。当该账号已有有效在线 access 会话时，新的登录请求应被拒绝，并提示“该账号已经登录”。

引入 refresh token 后，`login_accounts:{accountId}` 保存 refreshTokenId，便于刷新 access token 时保持账号占用索引稳定。但当前业务口径下，是否在线仍以 refresh 会话中的 currentTokenId 对应的 `login_tokens:{tokenId}` 是否存在为准。access 会话已不存在时，允许用户重新输入账号密码登录，并清理旧 refresh 会话。

## 目标

1. 同一 `sys_user.user_id` 只允许一个有效 Keystone 在线 access 会话。
...
| `login_accounts:{accountId}` | `refreshTokenId` | 保存账号当前 refresh 会话 ID；用于定位 currentTokenId 并判断同一账号是否已经在线 |

`accountId` 优先使用 `SystemLoginUser.userId`。如果未来出现无 `userId` 的 Keystone 登录主体，则兜底使用 `username`。

`login_tokens:{tokenId}` 使用 `token.expirationSeconds`；`login_refresh_tokens:{refreshTokenId}` 和 `login_accounts:{accountId}` 使用 `token.refreshExpirationSeconds`。账号是否在线不只看 `login_accounts` 是否存在，还要确认其指向的 refresh 会话里 `currentTokenId` 对应的 `login_tokens:{tokenId}` 仍存在。
...
3. 检查 `login_accounts:{accountId}`：
   ...
   - 如果存在，读取其中的旧 `refreshTokenId`，再读取 `login_refresh_tokens:{refreshTokenId}`，并绕过本地 Caffeine 缓存直接检查 Redis 中 `login_tokens:{currentTokenId}` 是否存在。
   - 如果旧 access token 仍在线，抛出 `Business.LOGIN_ACCOUNT_ALREADY_LOGGED_IN`。
   - 如果旧 refresh 会话不存在、已过期、已撤销，或旧 access token 已过期/不存在，删除残留 refresh 会话和账号标记，然后重新尝试原子占用。
...
Keystone JWT 是有明确过期时间的令牌。Redis 中 `login_tokens:{tokenId}` 使用 `token.expirationSeconds`；refresh 会话和账号占用使用 refresh token TTL。即使 refresh 会话仍未过期，只要 `login_tokens:{tokenId}` 已过期，用户就不再被视为在线。
...
3. 仅当账号占用值等于当前 refresh 会话 ID 时删除账号占用标记。

如果 `login_tokens:{tokenId}` 已经缺失，`TokenService` 会从 JWT claim 中读取 `login_refresh_session_id` / `login_user_id` / `login_username`，仍然尝试释放匹配的 refresh 会话和账号占用标记。最后一步避免误删其它会话的账号占用标记。
...
3. 如果能读到登录用户或 JWT/refresh 会话关联信息，则按正常退出规则释放 refresh 会话和账号占用标记。
...
1. 创建 token 时包含标准 JWT claims，access 会话使用 `token.expirationSeconds`，refresh 会话和账号占用使用 `token.refreshExpirationSeconds`。
2. 已有在线 access 会话时拒绝第二次登录。
3. 账号占用标记残留但 Redis 中旧 access 会话已过期或缺失时，绕过本地缓存确认后清理残留并允许登录。
